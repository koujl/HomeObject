...
#pragma pack(1)
    ...
    struct BlobHeader : DataHeader {
        ...
        uint8_t header_hash[blob_max_hash_len]{};
        ...
        bool valid() const {
            ...
            case HashAlgorithm::CRC32: {
                ...
                std::memcpy(hash_arr, header_hash, sizeof(uint32_t));
                std::memset(const_cast< uint8_t* >(header_hash), 0, blob_max_hash_len);
                uint32_t computed_hash = crc32_ieee(0, (uint8_t*)this, sizeof(BlobHeader));
                std::memcpy((void*)header_hash, hash_arr, sizeof(uint32_t));
                uint32_t hash;
                std::memcpy(&hash, hash_arr, sizeof(uint32_t));
        ...
        void seal() {
            std::memset(const_cast< uint8_t* >(header_hash), 0, blob_max_hash_len);
            uint32_t computed_hash = crc32_ieee(0, (uint8_t*)this, sizeof(BlobHeader));
            std::memcpy((void*)header_hash, &computed_hash, sizeof(uint32_t));
        }
    };
#pragma pack()
